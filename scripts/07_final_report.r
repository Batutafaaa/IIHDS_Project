...
cat(sprintf("   • Marginal effect: Each wealth unit reduces employment by %.1f pp%s\n", 
...
cat(sprintf("   • Organization membership: +%.1f pp employment probability%s\n", 
...
cat(sprintf("   • Education: +%.1f pp employment probability per year%s\n", 
...
cat(sprintf("   • Female employment penalty: %.1f percentage points%s\n", 
...
if (!is.na(muslim_marginal_effect)) {
  cat(sprintf("   • NET DISADVANTAGE: %.1f pp lower employment after controlling all factors%s\n", 
...
if(nrow(muslim_gender_gap) == 2) {
  ...
  cat(sprintf("   • Internal Gender Gap: %.1f pp (M: %.1f%%, F: %.1f%%)\n",
              100 * (male_emp - female_emp), 100 * male_emp, 100 * female_emp))
  cat(sprintf("   • Education Gender Gap: %.1f years (M: %.1f, F: %.1f)\n",
...
for (i in 1:nrow(ame_results)) {
  ...
  if(row$p_value < 0.05) {
    ...
    cat(sprintf("   • %s %s employment by %.1f pp\n", 
                row$Variable, direction, abs(row$AME_percentage)))
  }
}

if (!is.na(muslim_marginal_effect) && muslim_marginal_effect < 0) {
  cat(sprintf("   • Muslim identity decreases employment by %.1f pp (net of all factors)\n", 
              abs(muslim_marginal_effect)))
}

cat("\n9. DATA QUALITY & SAMPLE CHARACTERISTICS:\n")
cat(sprintf("   • Final analytic sample: %,d individuals\n", nrow(analysis_data)))
cat(sprintf("   • Employment rate: %.1f%%\n", 100 * mean(analysis_data$employed, na.rm = TRUE)))
cat(sprintf("   • Muslim sample size: %,d observations\n", 
            group_comparison$n[group_comparison$social_group == "Muslims"]))
